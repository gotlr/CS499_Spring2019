...
test_that("knn computes same answer as R", {
  data(zip.train, package = "ElemStatLearn")
    # Which data points are 0 or 1 in the first column?
  i01 <- which(zip.train[,1] %in% c(0,1)) # A vector of values
  train.i <- i01[1:5] # Vector of values from i01[1] to i01[5]
  test.i <- i01[6] # Integer value 14
